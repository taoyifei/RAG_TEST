...
validate_manual_target() {
  for path in "${candidate_env}" "${verified_state}" "${target_contract}"; do
    [[ -f "${path}" && ! -L "${path}" ]] || return 1
  done
  cmp -s -- "${candidate_env}" "${env_file}" || return 1
  verify_industry_app_identity "${candidate_env}" true || return 1
  local target_compose
  local runtime_state
  target_compose="$(industry_compose_file "${candidate_env}")" || return 1
  runtime_state="${transaction}/manual-rollback-target-runtime.json"
  run_industry_compose "${candidate_env}" "${target_compose}" \
    exec -T rag-industry-app rag-app runtime-state >"${runtime_state}" \
    || return 1
  chmod 600 "${runtime_state}" || return 1
  python3 "${script_dir}/runtime_check.py" validate-runtime-state \
    "${pre_index}" "${target_contract}" "${verified_state}" \
    "${update_manifest}" "${runtime_state}" >/dev/null || return 1
  python3 "${script_dir}/last_good.py" restore-source-pointer \
    ...
    --validate-only >/dev/null || return 1
  verify_index_identity "${candidate_env}" "${target_compose}" || return 1
  verify_dependency_identity || return 1
...
if [[ "${mode}" == "--manual-verified" ]]; then
  validate_manual_target \
    || rollback_abort "MANUAL_ROLLBACK_TARGET_PRECHECK_FAILED"
fi
write_industry_serving_transaction_state \
  "${transaction_state}" rolling_back "${update_id}" \
