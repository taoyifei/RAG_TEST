...
2026-09-26 统一后台接入后的复核：公开应用选用已核实的原生模型与 3 段重排设置；修复候选服务对内部 `rerank-adapter` 的 SSRF 白名单后，现场重问“固定资产从什么时候开始折旧？”两次均完成并给出 1 段引用。这说明该样本在当前配置下可完成，**不证明所有长检索、长历史或边界 token 请求不再越界**。最终送模计数与通用预算保护仍缺失，P0 保持未关闭。

## 2026-09-26 外壳验收范围调整

本记录前述“P0，原生预算修复阻塞”是早期将通用原生预算器列为发布前修复条件时的历史结论。当前任务只验湾事通外壳与原生服务的等价性；模型有限上下文属于已知原生组合限制，**不再要求湾事通开发或部署通用 tokenizer、近似裁剪或原生算法改造作为外壳验收条件**。旧 400 确实发生过，且网关记录为失败、原生消息未完成；当前同题已成功，不能写成持续复现，也不能据此宣布所有长度均已修复。

如后续短问答在当前配置下频繁出现同类 400，仍应核对模型实际请求和网关错误呈现，再判断是否出现新的接入配置缺陷。长历史、并发和恢复等独立现场门禁仍未通过；当前分类与证据见[外壳与原生等价性复核](WRAPPER-EQUIVALENCE-REVIEW.md)。
