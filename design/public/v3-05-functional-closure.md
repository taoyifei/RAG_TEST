...
| 本轮额外发现：Trace 性能 | SAFE 每个事件同步写两个 SQLite 路径；共享 History 在高并发下逐请求 commit；默认约 4 MiB WAL 自动 checkpoint 与周期 prune 会争用磁盘和 CPU，高并发 P95 显著退化且抖动 | History 的并发 STARTED/终态以逐命令 savepoint 和有界 group commit 落盘；非 FULL Trace 私有有界缓冲，完成时 single writer/单事务写 canonical Trace 与兼容事件；prune/manual checkpoint 仅在无活跃查询且 writer 持续经过短静默窗后运行，64 MiB 自动 checkpoint 作为有界后备；FULL 仍同步 fail-closed | durable commit 后才放行、单命令失败隔离、exact duration、事务回滚、维护与查询/flush 互斥竞态、WAL 后备水位、队列有界、capture complete、0 dropped 与冻结非劣阈值 |
...
普通观测至少 5 waves，NONE/SAFE P95 比较至少 40 个独立 waves、每 cell 至少
320 个样本。低并发 cell 在同一 Session 的每个独立 wave 内顺序执行多个并发
batch；各 batch 使用独立查询身份，且 A/B 相邻并交替先后，避免一个共享调度慢波
被误当成同批并发请求的多个独立尾样本。内存采样和 writer flush 均在计时区外，
矩阵在独立 pytest 子进程执行以隔离前序用例状态。

冻结阈值包括：HTTP error 为 0、SAFE dropped 为 0、identical cold 每 batch 只有一个
fresh execution、缓存不超过 512，以及 SAFE P95 相对 NONE 不劣于
`max(10%, 10ms)`。最终报告记录 88 项判定、11456 个计时 HTTP 请求、stream 首协议/
