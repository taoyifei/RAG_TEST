...
| 本轮额外发现：Trace 性能 | SAFE 每个事件同步写两个 SQLite 路径，且默认约 4 MiB WAL 自动 checkpoint 与周期 prune 会争用磁盘和 CPU，高并发 P95 显著退化 | 非 FULL 请求私有有界缓冲；完成时单 writer/单事务写 canonical Trace 与兼容事件；prune/manual checkpoint 仅在无活跃查询且 writer 持续经过短静默窗后运行，64 MiB 自动 checkpoint 作为有界后备；FULL 仍同步 fail-closed | exact duration、事务回滚、维护与查询/flush 互斥竞态、WAL 后备水位、队列有界、capture complete、0 dropped 与冻结非劣阈值 |
