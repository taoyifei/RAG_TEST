...
Python 和前端依赖分别默认由 pip-audit 与 npm audit 审计。只有输出明确表明 TLS、
超时、DNS、socket 或连接传输故障时，命令才会把对应路径标为 `BLOCKED`，并通过
OSV 官方批量接口实时检查锁文件内的全部精确版本。若原生审计已报告漏洞，或 OSV
传输、响应、结果不完整，命令立即失败，不允许用回退路径掩盖发现。
