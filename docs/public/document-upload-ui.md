...
1. “新建文档”调用 `POST .../documents`。即使 DOCX 字节相同，也创建新的逻辑
...
上传只发送浏览器选择的 DOCX 字节和 basename，不发送本地路径。每次写操作生成
Idempotency-Key；客户端不自动重放上传。413、422、409 等错误使用统一 Error Envelope
显示 code、stage、retryable 与 trace id 边界。
