...
class _WaveOutcome:
    ...
    pre_sample_gc: _PreSampleGc


@dataclass(frozen=True, slots=True)
class _PreSampleGc:
    """计时窗前回收测试夹具循环引用的可审计结果。"""
...
class _CellAccumulator:
    ...
    pre_sample_gc: list[dict[str, float | int]] = field(default_factory=list)
    ...
    def add(
        ...
        self.pre_sample_gc.append(asdict(outcome.pre_sample_gc))
...
def _load_thresholds() -> dict[str, object]:
    ...
    if payload.get("schema_version") != ("v3-05-product-http-thresholds-v5"):
    ...
    *,
    ...
    async with httpx.AsyncClient(
        ...
        try:
            ...
            pre_sample_gc = _collect_fixture_cycles()
    ...
    return _WaveOutcome(
        ...
        pre_sample_gc=pre_sample_gc,
    )


def _collect_fixture_cycles(
    collector: Callable[[], int] = gc.collect,
    clock: Callable[[], float] = time.perf_counter,
) -> _PreSampleGc:
    """在计时窗外清除 ASGI app/session 夹具留下的循环引用。

    性能矩阵为了保持每个 wave 的 HTTP 安全状态相互独立，会重新构造
    ASGI app。这些非生产控制面对象会积累数万个循环引用，并让
    generation-2 GC 随机落入某一侧的请求计时窗。这里在 app、Session、
    预热与 Trace flush 完成后统一回收；不清理 Product Runtime、cache、
    数据库或 Trace 状态。

    Args:
        collector: 可测试的 Python 循环垃圾收集入口。
        clock: 可测试的高精度单调时钟。
...
def _summarize_cell(  # noqa: PLR0913
    ...
    *,
    ...
    return {
        ...
        "pre_sample_gc": {
            "collected_total": sum(
                int(item["collected"]) for item in accumulator.pre_sample_gc
            ),
            "pause_ms_max": round(
                max(
                    float(item["elapsed_ms"])
                    for item in accumulator.pre_sample_gc
                ),
                3,
            ),
            ...
            "runs": len(accumulator.pre_sample_gc),
        },
...
def _run_matrix(tmp_path: Path) -> dict[str, object]:
    ...
    return {
        ...
        "environment": {
            ...
            "pre_sample_gc_control": (
                "full_collection_after_fixture_setup_before_timed_requests"
            ),
            "stream_transport": "Starlette TestClient ASGI HTTP",
        },
        "passed": passed
        and all(bool(item["final_seen"]) for item in stream_probes),
        "report_schema": "v3-05-product-http-performance-v5",
...
def test_pre_sample_gc_is_measured_outside_latency_window() -> None:
    """验证夹具 GC 控制可审计且不借用请求计时时钟。"""
    ticks = iter((10.0, 10.125))
    outcome = _collect_fixture_cycles(
        collector=lambda: 23_456,
        clock=lambda: next(ticks),
    )

    assert outcome == _PreSampleGc(collected=23_456, elapsed_ms=125.0)
