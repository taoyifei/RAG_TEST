...
“问答历史”按时间、知识库、结果和正文关键字查找请求，每条答案可“查看检索
过程”。产品默认在数据目录 SQLite 中持久化 Trace，并用既有主密钥加密问题及
答案，默认保留七天；关闭正文时只保留元数据。成功、拒答、缓存命中及模型失败
均显示实际调用与原因，重启时未结束的请求标记为中断。历史访问重新检查来源
与权限，删除来源后正文不再可见。浏览器断开连接不保证取消已经开始的模型请求。
