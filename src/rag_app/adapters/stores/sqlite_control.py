        ...
        *,
        ...
        with self._connections.transaction() as connection:
            rows = connection.execute(
                ...
                "AND (json_extract(s.value, '$.is_repeated')=0 "
                "OR EXISTS (SELECT 1 FROM "
                "json_each(c.chunk_json, '$.metadata') rm, "
                "json_each(rm.value, '$[1]') ra "
                "WHERE json_extract(rm.value, '$[0]')='atoms' "
                "AND json_extract(ra.value, '$.metadata.row_index') > "
                "json_extract(s.value, '$.source_anchor.row_index')))) "
                "AND EXISTS ("
