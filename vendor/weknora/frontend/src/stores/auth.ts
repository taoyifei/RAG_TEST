...
export const useAuthStore = defineStore('auth', () => {
  ...
  const gatewaySession = ref(false)
  ...
  const isLoggedIn = computed(() => {
    return (WANSHITONG_GATEWAY_AUTH ? gatewaySession.value : !!token.value) && !!user.value
  })
  ...
  const setUser = (userData: UserInfo) => {
    ...
    user.value = userData
    // 保存到localStorage
    if (!WANSHITONG_GATEWAY_AUTH) localStorage.setItem('weknora_user', JSON.stringify(userData))
  ...
  const setTenant = (tenantData: TenantInfo | null) => {
    ...
    if (WANSHITONG_GATEWAY_AUTH) {
      // 管理身份只来自本次网关 bootstrap，不缓存另一账号的空间。
    } else if (tenantData) {
  ...
  const setMemberships = (
    ...
    if (!WANSHITONG_GATEWAY_AUTH) localStorage.setItem('weknora_memberships', JSON.stringify(memberships.value))
  }
  ...
  const refreshFromAuthMe = async (): Promise<boolean> => {
    try {
      ...
      if (!response.success || !u) {
        if (WANSHITONG_GATEWAY_AUTH) gatewaySession.value = false
        return false
      }
      ...
      if (WANSHITONG_GATEWAY_AUTH) gatewaySession.value = true
  ...
  const initFromStorage = () => {
    if (WANSHITONG_GATEWAY_AUTH) {
      // 旧版原生令牌即使留在同源 localStorage 也不进入网关模式。
      localStorage.removeItem('weknora_token')
      localStorage.removeItem('weknora_refresh_token')
      localStorage.removeItem('weknora_user')
      localStorage.removeItem('weknora_tenant')
