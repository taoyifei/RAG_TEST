...
import { WANSHITONG_GATEWAY_AUTH, redirectToWanshitongAdminLogin } from '@/config/wanshitongGateway'
import {
  liveInstallPercent,
  progressKey,
...
export function useConfigSkillInstallProgress(options?: {
  ...
  function follow(configId: string, skillId: string) {
    ...
    void fetchEventSource(url, {
      method: 'GET',
      headers: {
        ...(!WANSHITONG_GATEWAY_AUTH && token ? { Authorization: `Bearer ${token}` } : {}),
      ...
      async onopen(response) {
        if (WANSHITONG_GATEWAY_AUTH && response.status === 401) {
          redirectToWanshitongAdminLogin()
        }
        if (!response.ok) throw new Error(`skill install stream refused: ${response.status}`)
      },
      onmessage(ev) {
