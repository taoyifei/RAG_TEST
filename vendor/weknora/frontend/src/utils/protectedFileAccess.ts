...
import { WANSHITONG_GATEWAY_AUTH, engineResourceUrl } from '../config/wanshitongGateway';
...
export function isProtectedFileProxyPath(pathname: string): boolean {
  const resourcePath = WANSHITONG_GATEWAY_AUTH
    ? pathname.replace(/^\/kb\/api\/engine-admin(?=\/)/, '')
    : pathname;
  return (
    resourcePath === '/files'
    || KB_FILE_PROXY_PATH_RE.test(resourcePath)
    || MESSAGE_FILE_PROXY_PATH_RE.test(resourcePath)
    || EMBED_FILE_PROXY_PATH_RE.test(resourcePath)
  );
}
...
function tenantRequestHeaders(): Record<string, string> {
  ...
  try {
    const token = WANSHITONG_GATEWAY_AUTH ? '' : (localStorage.getItem('weknora_token') || '').trim();
    if (token) {
...
export function buildProtectedFileRequest(
  ...
  if (access.mode === 'embed') {
    ...
    return {
      url: engineResourceUrl(`/api/v1/embed/${encodeURIComponent(channelId)}/files?${query}`),
  ...
  if (access.mode === 'knowledgeBase') {
    return {
      url: engineResourceUrl(`/api/v1/knowledge-bases/${encodeURIComponent(access.kbId.trim())}/files?${query}`),
  ...
  if (access.mode === 'message') {
    return {
      url: engineResourceUrl(`/api/v1/sessions/${encodeURIComponent(access.sessionId.trim())}/messages/${encodeURIComponent(access.messageId.trim())}/files?${query}`),
      headers: tenantRequestHeaders(),
    };
  }

  return { url: engineResourceUrl(`/files?${query}`), headers: tenantRequestHeaders() };
}
