      ...
      <template v-for="group in navGroups" :key="group.key">
        ...
        <template v-for="item in group.items" :key="item.key">
          <div :class="['nav-item', {
            ...
            <svg v-else-if="item.key === 'weknoracloud' && !WANSHITONG_GATEWAY_AUTH" width="17" height="17" viewBox="0 0 18 18"
...
const isSectionSupported = (key: string): boolean => {
  if (WANSHITONG_GATEWAY_AUTH && isIntegrationSection(key)
    && (integrationTabFromSection(key) === 'im' || integrationTabFromSection(key) === 'embed')) {
    return false
  }
  if (WANSHITONG_GATEWAY_AUTH && (
    key === 'integration-api' || key === 'platform-api-keys' ||
    key === 'userprofile' || key === 'mymemory' || key === 'envvars' || key === 'sandbox'
  )) {
    return false
  }
...
const navItems = computed(() => {
  ...
  const all: NavItem[] = [
    ...
    { key: 'weknoracloud', icon: WANSHITONG_GATEWAY_AUTH ? 'cloud' : '', label: WANSHITONG_GATEWAY_AUTH ? '云模型服务' : 'WeKnora Cloud' },
