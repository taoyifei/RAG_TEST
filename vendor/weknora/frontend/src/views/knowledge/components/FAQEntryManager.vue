...
import { engineResourceUrl, WANSHITONG_GATEWAY_AUTH } from '@/config/wanshitongGateway'
...
const downloadFailedEntries = () => {
  ...
  // 直接打开下载链接
  window.open(engineResourceUrl(importResult.value.failed_entries_url), '_blank', 'noopener,noreferrer')
}
...
const exampleData: FAQEntryPayload[] = [
  {
    ...
    answers: [`${exampleProductName} 是一个智能知识库管理系统`, '它支持多种知识库类型和导入方式'],
    similar_questions: [`${exampleProductName} 是什么？`, `介绍一下 ${exampleProductName}`],
    negative_questions: [`这不是 ${exampleProductName}`, `与 ${exampleProductName} 无关`],
    tag_name: '产品介绍',
  },
  {
